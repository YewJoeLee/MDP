...
def align_command(obstacle, goal_pose):
    ...
    facing = goal_pose[2]
    dx = abs(goal_pose[0] - obstacle[1])
    dy = abs(goal_pose[1] - obstacle[2])

    if facing in ("N", "S"):
        cells_away, sideways = dy, dx
    else:
        cells_away, sideways = dx, dy

    if sideways != 0 and not config.ALIGN_ON_OFFSET:
        return None

    if not config.ALIGN_SEND_DISTANCE:
        return config.ALIGN_PREFIX

    target_cm = cells_away * config.ALIGN_CM_PER_CELL + config.ALIGN_OFFSET_CM

    if target_cm <= 0:
        return None

    return config.ALIGN_PREFIX + str(int(round(target_cm)))
...
def plan(obstacles, start_pose=config.START_POSE):
    ...
    obstacle_by_id = {obstacle[0]: obstacle for obstacle in obstacles}
    ...
    for obstacle_id, goal_pose in best_order:
        ...
        align = align_command(obstacle_by_id[obstacle_id], goal_pose)
        if align is not None:
            segment_commands.append(align)
